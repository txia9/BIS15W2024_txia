...
```r
amoniata <- read.csv("data/amniota.csv")
```
...
```r
amoniata %>% 
  map_df(~ sum(is.na(.))) #this does not make sense
```
...
```r
amphibio %>% 
  map_df(~ sum(is.na(.))) #this one does make sense
```
...
```r
amoniata <- read.csv("data/amniota.csv", na=c("-999", "-999.00", "-999.00000","-999.0", "-999.000")) %>% clean_names()
```

**6. Use the package `naniar` to produce a summary, including percentages, of missing data in each column for the `amniota` data.**  

```r
naniar::miss_var_summary(amoniata)
```
...
it makes sense, because mammals does not have eggs, so the percentage of na is 100.  
...
These NA represents different habitat of each species. For example, an arboreal species is more likely to be arboreal instead of aquatic.  
