...
large <- data.frame(large_mammals)
small <- data.frame(small_mammals)
large
```
...
Large animals sleep 3.3 hrs on average, and small animals sleep 11.8 hrs on average, so small animals sleep longer.  
...
```
Little brown bat is the sleepiest with 19.9 hrs sleep among the entire dataframe. 
