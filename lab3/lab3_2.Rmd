...
change the class of data: from character to factors
```{r}
...
levels(hot_springs$spring) #null means spring is still a character not character, unable to find the factors.  
```
...
```{r}
table(fish$lakeid) #will not work if data is too big
```
