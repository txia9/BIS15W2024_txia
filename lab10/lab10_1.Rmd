...
```{r}
ggplot(data=homerange, mapping=aes(x=log10.mass, y=log10.hra))+ #+ sign means adding a layer
...
ggplot(data=homerange, mapping=aes(x=log10.mass, y=log10.hra))+
  geom_point()+
  geom_smooth(method=lm, se=T) #add a regression line (general base model)
```
...
homerange %>% 
  ...
  ggplot(aes(x=common.name, y=log10.mass))+
  geom_col()+
  coord_flip() #does not actually work in class
```
