...
fish %>% # work with the fish data
  filter(lakeid == "AL" | lakeid == "AR") %>% #oly these lakes
  filter(between(radii_length_mm, 2, 4)) %>% #between 2 and 4
  arrange(desc(radii_length_mm)) #sort to make easier to read, arrange is from smal to largest, desc is from largest to smallest
```
