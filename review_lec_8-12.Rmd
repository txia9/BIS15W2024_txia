...
#life_history %>% 
  ...
  ggplot(aes(x=reorder(order, mean_mass), y=mean_mass))+ #this allow us to order the x-axis by mean_mass. reorder(x, y)
  geom_col()+
  coord_flip()+
...
+`scale_colour_brewer(palette="palette_name")` is for points  
+`scale_fill_brewer()` is for fills  
```

facet -- spliting box plot.  
```{r}
homerange %>% 
  ggplot(aes(x=taxon, y=log10.mass))+
  geom_boxplot()+
  facet_wrap(~trophic.guild)+ #split into colomn (left and right)
  or facet_grid(trophic.guild~.)+ #split into rows(up and down)
  or facet_grid(trophic.guild~thermoregulation)+ #a~b where a is rows and b is columns.  
