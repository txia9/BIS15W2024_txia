...
```{r}
files <- list.files(path = "data/spiders", pattern = ".csv", full.names = TRUE) #full.names show the result including the path
files
```
...
`inner_join`  
Where did customers 3 and 4 go? #only join the row that appear in both data. 
```{r}
inner_exampleDF <- inner_join(table_A, table_B, by="customer_ID") #only join the row that appear in both data. 
inner_exampleDF
```
...
```{r}
left_exampleDF <- left_join(table_A, table_B, by="customer_ID") #join second table into first. 
left_exampleDF
```
...
```{r}
right_exampleDF <- right_join(table_A, table_B, by="customer_ID") #join first table into second. 
right_exampleDF
```

`full_join`
```{r}
full_exampleDF <- full_join(table_A, table_B, by="customer_ID") #join both table entirely, and keep NAs. 
full_exampleDF
```
