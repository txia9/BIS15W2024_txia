...
###this is not tidy, because the drug is the observation. To make it tidy, we need to make "abcd" into a column.  
...
it is easier for R to read the tidy data.  
...
plant_data2 <- 
  plant_data %>% 
    pivot_longer(
    cols = starts_with("day"), #columns that start with "wk"
    names_to = "day",
    names_prefix = "day",
    values_to = "rank",
    values_drop_na = TRUE)
